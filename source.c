...
void TIPWindow::SetupGraph(TDC &dc)
{
  ...
  yGMin = 3 * cy;
  yGMax = yMax - 3 * cy; // Allows 4 lines of text to be displayed on top and bottom
...
void ace()
{
   ...
   * Weights for ACE(vt), and prediction value (pred)
   */
  double vtsum = 0;
  int i, j;
...
void ase()
{
  ...
  {
    // Sigmoidal Function (between +1 and 1)
    action = 2 * (1 / (1 + exp(-dom)) - 0.50);
  }
  else
  {
    // Bang Bang Output
